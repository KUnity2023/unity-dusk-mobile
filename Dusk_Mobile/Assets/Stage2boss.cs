...
public class Stage2boss : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        meleeRight.transform.localPosition.Set(0.16f,-0.025f,0);
        meleeLeft.transform.localPosition.Set(-0.16f,-0.025f,0);
    ...
    public void phase3init(){
        meleeRight.transform.localPosition.Set(0.16f,-0.1f,0);
        meleeLeft.transform.localPosition.Set(-0.16f,-0.1f,0);
    }
    public void phase3Atk2(){
        meleeLeft.GetComponent<BoxCollider2D>().enabled = true;
        meleeLeft.GetComponent<BoxCollider2D>().size.Set(0.32f,0.07f);
        meleeRight.GetComponent<BoxCollider2D>().enabled = true;
        meleeRight.GetComponent<BoxCollider2D>().size.Set(0.32f,0.07f);
    }
    public void phase3Atk3(){
        Collider2D colInfo = Physics2D.OverlapCircle(transform.position,2.7f,attackMask);    
        if(colInfo != null){
            colInfo.GetComponent<CharacterStats>().TakeDamage(attackDamage);
            colInfo.GetComponent<CharacterManager>().OnDamaged(transform.position);
            //맞은 대상의 레이어를 잠시 바꾸고 일정시간 데미지가 안들어가도록
    ...
    public void phase3heal(){
        GetComponent<CharacterStats>().curHealth += 50;
    }
